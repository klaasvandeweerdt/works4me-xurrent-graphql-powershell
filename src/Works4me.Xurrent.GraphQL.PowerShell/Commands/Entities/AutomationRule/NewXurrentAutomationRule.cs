...
{
    /// <summary>
    /// Creates a new <see cref="AutomationRule"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="AutomationRuleCreateInput"/> from the provided parameters, executes the operation, and returns a <see cref="AutomationRuleCreatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.New, "XurrentAutomationRule")]
    [OutputType(typeof(AutomationRuleCreatePayload))]
    public class NewXurrentAutomationRule : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="AutomationRuleQuery"/> that defines which fields of the <see cref="AutomationRuleCreatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="AutomationRuleCreateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="AutomationRuleCreatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            AutomationRuleCreateInput input = new();
            ...
            try
            {
                ...
                AutomationRuleCreatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentAutomationRule), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentAutomationRule), ErrorCategory.NotSpecified, this));
