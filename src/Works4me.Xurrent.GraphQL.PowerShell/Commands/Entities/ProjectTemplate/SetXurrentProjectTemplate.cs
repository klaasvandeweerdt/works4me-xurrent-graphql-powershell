...
{
    /// <summary>
    /// Updates an existing <see cref="ProjectTemplate"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="ProjectTemplateUpdateInput"/> from the provided parameters, executes the operation, and returns a <see cref="ProjectTemplateUpdatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "XurrentProjectTemplate")]
    [OutputType(typeof(ProjectTemplateUpdatePayload))]
    public class SetXurrentProjectTemplate : XurrentCmdletBase
    {
        /// <summary>
        ...
        public string[]? PhasesToDelete { get; set; }
        ...
        /// <summary>
        /// Specifies the <see cref="ProjectTemplateQuery"/> that defines which fields of the <see cref="ProjectTemplateUpdatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="ProjectTemplateUpdateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="ProjectTemplateUpdatePayload"/> to the pipeline.<br/>
        ...
        protected override void OnProcessRecord()
        {
            ...
            if (MyInvocation.BoundParameters.ContainsKey(nameof(PhasesToDelete)))
                input.PhasesToDelete = PhasesToDelete is null ? new() : new(PhasesToDelete);
            ...
            try
            {
                ...
                ProjectTemplateUpdatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentProjectTemplate), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentProjectTemplate), ErrorCategory.NotSpecified, this));
