...
{
    /// <summary>
    /// Creates a new <see cref="Reservation"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="ReservationCreateInput"/> from the provided parameters, executes the operation, and returns a <see cref="ReservationCreatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.New, "XurrentReservation")]
    [OutputType(typeof(ReservationCreatePayload))]
    public class NewXurrentReservation : XurrentCmdletBase
    {
        /// <summary>
        ...
        [ValidateNotNullOrEmpty]
        public string ReservationOfferingId { get; set; } = string.Empty;
        ...
        /// <summary>
        /// Specifies the <see cref="ReservationQuery"/> that defines which fields of the <see cref="ReservationCreatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="ReservationCreateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="ReservationCreatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            ReservationCreateInput input = new();
            ...
            try
            {
                ...
                ReservationCreatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentReservation), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentReservation), ErrorCategory.NotSpecified, this));
