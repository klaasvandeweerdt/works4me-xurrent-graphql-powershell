...
{
    /// <summary>
    /// Creates a new <see cref="Release"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="ReleaseCreateInput"/> from the provided parameters, executes the operation, and returns a <see cref="ReleaseCreatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.New, "XurrentRelease")]
    [OutputType(typeof(ReleaseCreatePayload))]
    public class NewXurrentRelease : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="ReleaseQuery"/> that defines which fields of the <see cref="ReleaseCreatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="ReleaseCreateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="ReleaseCreatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            ReleaseCreateInput input = new();
            ...
            try
            {
                ...
                ReleaseCreatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentRelease), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentRelease), ErrorCategory.NotSpecified, this));
