...
{
    /// <summary>
    /// Creates a new <see cref="Risk"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="RiskCreateInput"/> from the provided parameters, executes the operation, and returns a <see cref="RiskCreatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.New, "XurrentRisk")]
    [OutputType(typeof(RiskCreatePayload))]
    public class NewXurrentRisk : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="RiskQuery"/> that defines which fields of the <see cref="RiskCreatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="RiskCreateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="RiskCreatePayload"/> to the pipeline.<br/>
        ...
        protected override void OnProcessRecord()
        {
            ...
            try
            {
                ...
                RiskCreatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentRisk), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentRisk), ErrorCategory.NotSpecified, this));
