...
{
    /// <summary>
    /// Updates an existing <see cref="ReservationOffering"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="ReservationOfferingUpdateInput"/> from the provided parameters, executes the operation, and returns a <see cref="ReservationOfferingUpdatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "XurrentReservationOffering")]
    [OutputType(typeof(ReservationOfferingUpdatePayload))]
    public class SetXurrentReservationOffering : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="ReservationOfferingQuery"/> that defines which fields of the <see cref="ReservationOfferingUpdatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="ReservationOfferingUpdateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="ReservationOfferingUpdatePayload"/> to the pipeline.<br/>
        ...
        protected override void OnProcessRecord()
        {
            ...
            try
            {
                ...
                ReservationOfferingUpdatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentReservationOffering), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentReservationOffering), ErrorCategory.NotSpecified, this));
