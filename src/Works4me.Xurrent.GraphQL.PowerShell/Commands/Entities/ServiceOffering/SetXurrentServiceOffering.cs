...
{
    /// <summary>
    /// Updates an existing <see cref="ServiceOffering"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="ServiceOfferingUpdateInput"/> from the provided parameters, executes the operation, and returns a <see cref="ServiceOfferingUpdatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "XurrentServiceOffering")]
    [OutputType(typeof(ServiceOfferingUpdatePayload))]
    public class SetXurrentServiceOffering : XurrentCmdletBase
    {
        /// <summary>
        ...
        public string[]? EffortClassRatesToDelete { get; set; }
        ...
        public string[]? RfcTypeRatesToDelete { get; set; }
        ...
        public string[]? StandardServiceRequestsToDelete { get; set; }
        ...
        /// </summary>
        [Parameter(Mandatory = false, Position = 73, ValueFromPipelineByPropertyName = true)]
        ...
        /// </summary>
        [Parameter(Mandatory = false, Position = 74, ValueFromPipelineByPropertyName = true)]
        public string? TimeZone { get; set; }

        /// <summary>
        /// Specifies the <see cref="ServiceOfferingQuery"/> that defines which fields of the <see cref="ServiceOfferingUpdatePayload"/> are returned by the mutation.<br/>
        /// If omitted, a default selection is used.<br/>
        /// </summary>
        [Parameter(Mandatory = false, Position = 75, ValueFromPipelineByPropertyName = true)]
        ...
        /// </summary>
        [Parameter(Mandatory = false, Position = 76, ValueFromPipelineByPropertyName = true)]
        [ValidateNotNull]
        public XurrentPowerShellClient? Client { get; set; }

        /// <summary>
        /// Executes the mutation by constructing a <see cref="ServiceOfferingUpdateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="ServiceOfferingUpdatePayload"/> to the pipeline.<br/>
        ...
        protected override void OnProcessRecord()
        {
            ...
            if (MyInvocation.BoundParameters.ContainsKey(nameof(EffortClassRatesToDelete)))
                input.EffortClassRatesToDelete = EffortClassRatesToDelete is null ? new() : new(EffortClassRatesToDelete);
            ...
            if (MyInvocation.BoundParameters.ContainsKey(nameof(RfcTypeRatesToDelete)))
                input.RfcTypeRatesToDelete = RfcTypeRatesToDelete is null ? new() : new(RfcTypeRatesToDelete);
            ...
            if (MyInvocation.BoundParameters.ContainsKey(nameof(StandardServiceRequestsToDelete)))
                input.StandardServiceRequestsToDelete = StandardServiceRequestsToDelete is null ? new() : new(StandardServiceRequestsToDelete);
            ...
            try
            {
                ...
                ServiceOfferingUpdatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentServiceOffering), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentServiceOffering), ErrorCategory.NotSpecified, this));
