...
{
    /// <summary>
    /// Updates an existing <see cref="ShortUrl"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="ShortUrlUpdateInput"/> from the provided parameters, executes the operation, and returns a <see cref="ShortUrlUpdatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "XurrentShortUrl")]
    [OutputType(typeof(ShortUrlUpdatePayload))]
    public class SetXurrentShortUrl : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="ShortUrlQuery"/> that defines which fields of the <see cref="ShortUrlUpdatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="ShortUrlUpdateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="ShortUrlUpdatePayload"/> to the pipeline.<br/>
        ...
        protected override void OnProcessRecord()
        {
            ...
            try
            {
                ...
                ShortUrlUpdatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentShortUrl), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentShortUrl), ErrorCategory.NotSpecified, this));
