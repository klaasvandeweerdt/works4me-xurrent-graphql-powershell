...
{
    /// <summary>
    /// Updates an existing <see cref="ServiceLevelAgreement"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="ServiceLevelAgreementUpdateInput"/> from the provided parameters, executes the operation, and returns a <see cref="ServiceLevelAgreementUpdatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "XurrentServiceLevelAgreement")]
    [OutputType(typeof(ServiceLevelAgreementUpdatePayload))]
    public class SetXurrentServiceLevelAgreement : XurrentCmdletBase
    {
        /// <summary>
        ...
        public string[]? EffortClassRateIDsToDelete { get; set; }
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="ServiceLevelAgreementUpdateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="ServiceLevelAgreementUpdatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            ServiceLevelAgreementUpdateInput input = new();
            ...
            if (MyInvocation.BoundParameters.ContainsKey(nameof(EffortClassRateIDsToDelete)))
                input.EffortClassRateIDsToDelete = EffortClassRateIDsToDelete is null ? new() : new(EffortClassRateIDsToDelete);
            ...
            if (MyInvocation.BoundParameters.ContainsKey(nameof(StandardServiceRequestActivityIDsToDelete)))
                input.StandardServiceRequestActivityIDsToDelete = StandardServiceRequestActivityIDsToDelete is null ? new() : new(StandardServiceRequestActivityIDsToDelete);
            ...
            try
            {
                ...
                ServiceLevelAgreementUpdatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentServiceLevelAgreement), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentServiceLevelAgreement), ErrorCategory.NotSpecified, this));
