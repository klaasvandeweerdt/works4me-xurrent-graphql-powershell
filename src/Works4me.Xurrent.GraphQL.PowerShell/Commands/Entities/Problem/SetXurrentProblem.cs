...
{
    /// <summary>
    /// Updates an existing <see cref="Problem"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="ProblemUpdateInput"/> from the provided parameters, executes the operation, and returns a <see cref="ProblemUpdatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "XurrentProblem")]
    [OutputType(typeof(ProblemUpdatePayload))]
    public class SetXurrentProblem : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="ProblemQuery"/> that defines which fields of the <see cref="ProblemUpdatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="ProblemUpdateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="ProblemUpdatePayload"/> to the pipeline.<br/>
        ...
        protected override void OnProcessRecord()
        {
            ...
            try
            {
                ...
                ProblemUpdatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentProblem), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentProblem), ErrorCategory.NotSpecified, this));
