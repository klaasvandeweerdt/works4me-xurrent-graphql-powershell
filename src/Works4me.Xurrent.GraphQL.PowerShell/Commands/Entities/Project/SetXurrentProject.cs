...
{
    /// <summary>
    /// Updates an existing <see cref="Project"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="ProjectUpdateInput"/> from the provided parameters, executes the operation, and returns a <see cref="ProjectUpdatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "XurrentProject")]
    [OutputType(typeof(ProjectUpdatePayload))]
    public class SetXurrentProject : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="ProjectQuery"/> that defines which fields of the <see cref="ProjectUpdatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="ProjectUpdateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="ProjectUpdatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            ProjectUpdateInput input = new();
            ...
            try
            {
                ...
                ProjectUpdatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentProject), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentProject), ErrorCategory.NotSpecified, this));
