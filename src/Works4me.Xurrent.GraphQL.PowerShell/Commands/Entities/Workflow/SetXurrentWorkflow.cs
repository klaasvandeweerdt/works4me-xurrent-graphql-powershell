...
{
    /// <summary>
    /// Updates an existing <see cref="Workflow"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="WorkflowUpdateInput"/> from the provided parameters, executes the operation, and returns a <see cref="WorkflowUpdatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "XurrentWorkflow")]
    [OutputType(typeof(WorkflowUpdatePayload))]
    public class SetXurrentWorkflow : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="WorkflowQuery"/> that defines which fields of the <see cref="WorkflowUpdatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="WorkflowUpdateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="WorkflowUpdatePayload"/> to the pipeline.<br/>
        ...
        protected override void OnProcessRecord()
        {
            ...
            try
            {
                ...
                WorkflowUpdatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentWorkflow), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentWorkflow), ErrorCategory.NotSpecified, this));
