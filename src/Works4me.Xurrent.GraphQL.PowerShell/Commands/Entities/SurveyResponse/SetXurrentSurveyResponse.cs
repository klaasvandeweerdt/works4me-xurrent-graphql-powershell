...
{
    /// <summary>
    /// Updates an existing <see cref="SurveyResponse"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="SurveyResponseUpdateInput"/> from the provided parameters, executes the operation, and returns a <see cref="SurveyResponseUpdatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "XurrentSurveyResponse")]
    [OutputType(typeof(SurveyResponseUpdatePayload))]
    public class SetXurrentSurveyResponse : XurrentCmdletBase
    {
        /// <summary>
        ...
        public string[]? AnswersToDelete { get; set; }
        ...
        /// <summary>
        /// Specifies the <see cref="SurveyResponseQuery"/> that defines which fields of the <see cref="SurveyResponseUpdatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="SurveyResponseUpdateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="SurveyResponseUpdatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            SurveyResponseUpdateInput input = new();

            if (MyInvocation.BoundParameters.ContainsKey(nameof(Id)))
                input.Id = Id;

            if (MyInvocation.BoundParameters.ContainsKey(nameof(AnswersToDelete)))
                input.AnswersToDelete = AnswersToDelete is null ? new() : new(AnswersToDelete);
            ...
            try
            {
                ...
                SurveyResponseUpdatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentSurveyResponse), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentSurveyResponse), ErrorCategory.NotSpecified, this));
