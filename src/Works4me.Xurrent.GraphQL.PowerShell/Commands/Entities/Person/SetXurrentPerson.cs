...
{
    /// <summary>
    /// Updates an existing <see cref="Person"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="PersonUpdateInput"/> from the provided parameters, executes the operation, and returns a <see cref="PersonUpdatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "XurrentPerson")]
    [OutputType(typeof(PersonUpdatePayload))]
    public class SetXurrentPerson : XurrentCmdletBase
    {
        /// <summary>
        ...
        public string[]? AccountPermissionsToDelete { get; set; }
        ...
        public string[]? ContactsToDelete { get; set; }
        ...
        /// <summary>
        /// Specifies the <see cref="PersonQuery"/> that defines which fields of the <see cref="PersonUpdatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="PersonUpdateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="PersonUpdatePayload"/> to the pipeline.<br/>
        ...
        protected override void OnProcessRecord()
        {
            ...
            if (MyInvocation.BoundParameters.ContainsKey(nameof(AccountPermissionsToDelete)))
                input.AccountPermissionsToDelete = AccountPermissionsToDelete is null ? new() : new(AccountPermissionsToDelete);
            ...
            if (MyInvocation.BoundParameters.ContainsKey(nameof(ContactsToDelete)))
                input.ContactsToDelete = ContactsToDelete is null ? new() : new(ContactsToDelete);
            ...
            try
            {
                ...
                PersonUpdatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentPerson), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentPerson), ErrorCategory.NotSpecified, this));
