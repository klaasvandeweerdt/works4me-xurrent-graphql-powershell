...
{
    /// <summary>
    /// Updates an existing <see cref="Invoice"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="InvoiceUpdateInput"/> from the provided parameters, executes the operation, and returns a <see cref="InvoiceUpdatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "XurrentInvoice")]
    [OutputType(typeof(InvoiceUpdatePayload))]
    public class SetXurrentInvoice : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="InvoiceQuery"/> that defines which fields of the <see cref="InvoiceUpdatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="InvoiceUpdateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="InvoiceUpdatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            InvoiceUpdateInput input = new();
            ...
            try
            {
                ...
                InvoiceUpdatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentInvoice), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentInvoice), ErrorCategory.NotSpecified, this));
