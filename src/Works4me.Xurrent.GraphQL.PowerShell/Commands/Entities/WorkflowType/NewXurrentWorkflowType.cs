...
{
    /// <summary>
    /// Creates a new <see cref="WorkflowType"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="WorkflowTypeCreateInput"/> from the provided parameters, executes the operation, and returns a <see cref="WorkflowTypeCreatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.New, "XurrentWorkflowType")]
    [OutputType(typeof(WorkflowTypeCreatePayload))]
    public class NewXurrentWorkflowType : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="WorkflowTypeQuery"/> that defines which fields of the <see cref="WorkflowTypeCreatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="WorkflowTypeCreateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="WorkflowTypeCreatePayload"/> to the pipeline.<br/>
        ...
        protected override void OnProcessRecord()
        {
            ...
            try
            {
                ...
                WorkflowTypeCreatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentWorkflowType), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentWorkflowType), ErrorCategory.NotSpecified, this));
