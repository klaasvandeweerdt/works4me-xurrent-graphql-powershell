...
{
    /// <summary>
    /// Creates a new <see cref="SkillPool"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="SkillPoolCreateInput"/> from the provided parameters, executes the operation, and returns a <see cref="SkillPoolCreatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.New, "XurrentSkillPool")]
    [OutputType(typeof(SkillPoolCreatePayload))]
    public class NewXurrentSkillPool : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="SkillPoolQuery"/> that defines which fields of the <see cref="SkillPoolCreatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="SkillPoolCreateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="SkillPoolCreatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            SkillPoolCreateInput input = new();
            ...
            try
            {
                ...
                SkillPoolCreatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentSkillPool), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentSkillPool), ErrorCategory.NotSpecified, this));
