...
{
    /// <summary>
    /// Creates a new <see cref="RiskSeverity"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="RiskSeverityCreateInput"/> from the provided parameters, executes the operation, and returns a <see cref="RiskSeverityCreatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.New, "XurrentRiskSeverity")]
    [OutputType(typeof(RiskSeverityCreatePayload))]
    public class NewXurrentRiskSeverity : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="RiskSeverityQuery"/> that defines which fields of the <see cref="RiskSeverityCreatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="RiskSeverityCreateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="RiskSeverityCreatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            RiskSeverityCreateInput input = new();
            ...
            try
            {
                ...
                RiskSeverityCreatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentRiskSeverity), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentRiskSeverity), ErrorCategory.NotSpecified, this));
