...
{
    /// <summary>
    /// Creates a new <see cref="ProjectTask"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="ProjectTaskCreateInput"/> from the provided parameters, executes the operation, and returns a <see cref="ProjectTaskCreatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.New, "XurrentProjectTask")]
    [OutputType(typeof(ProjectTaskCreatePayload))]
    public class NewXurrentProjectTask : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="ProjectTaskQuery"/> that defines which fields of the <see cref="ProjectTaskCreatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="ProjectTaskCreateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="ProjectTaskCreatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            ProjectTaskCreateInput input = new();
            ...
            try
            {
                ...
                ProjectTaskCreatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentProjectTask), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentProjectTask), ErrorCategory.NotSpecified, this));
