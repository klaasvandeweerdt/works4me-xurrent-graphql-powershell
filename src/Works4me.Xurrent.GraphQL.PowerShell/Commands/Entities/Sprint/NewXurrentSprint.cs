...
{
    /// <summary>
    /// Creates a new <see cref="Sprint"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="SprintCreateInput"/> from the provided parameters, executes the operation, and returns a <see cref="SprintCreatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.New, "XurrentSprint")]
    [OutputType(typeof(SprintCreatePayload))]
    public class NewXurrentSprint : XurrentCmdletBase
    {
        /// <summary>
        ...
        public string ScrumWorkspaceId { get; set; } = string.Empty;
        ...
        /// <summary>
        /// Specifies the <see cref="SprintQuery"/> that defines which fields of the <see cref="SprintCreatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="SprintCreateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="SprintCreatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            SprintCreateInput input = new();
            ...
            try
            {
                ...
                SprintCreatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentSprint), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentSprint), ErrorCategory.NotSpecified, this));
