...
{
    /// <summary>
    /// Creates a new <see cref="Broadcast"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="BroadcastCreateInput"/> from the provided parameters, executes the operation, and returns a <see cref="BroadcastCreatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.New, "XurrentBroadcast")]
    [OutputType(typeof(BroadcastCreatePayload))]
    public class NewXurrentBroadcast : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Specifies the <see cref="BroadcastQuery"/> that defines which fields of the <see cref="BroadcastCreatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="BroadcastCreateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="BroadcastCreatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            BroadcastCreateInput input = new();
            ...
            try
            {
                ...
                BroadcastCreatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentBroadcast), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(NewXurrentBroadcast), ErrorCategory.NotSpecified, this));
