...
namespace Works4me.Xurrent.GraphQL.PowerShell.Commands
{
    ...
    /// This cmdlet constructs a <see cref="RequestUpdateInput"/> from the provided parameters, executes the operation, and returns a <see cref="RequestUpdatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "XurrentRequest")]
    [OutputType(typeof(RequestUpdatePayload))]
    public class SetXurrentRequest : XurrentCmdletBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="RequestUpdateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="RequestUpdatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            RequestUpdateInput input = new();
            ...
            if (MyInvocation.BoundParameters.ContainsKey(nameof(TagsToDelete)))
                input.TagsToDelete = TagsToDelete is null ? new() : new(TagsToDelete);
            ...
            if (MyInvocation.BoundParameters.ContainsKey(nameof(WatchesToDelete)))
                input.WatchesToDelete = WatchesToDelete is null ? new() : new(WatchesToDelete);
            ...
            try
            {
                ...
                RequestUpdatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentRequest), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentRequest), ErrorCategory.NotSpecified, this));
