...
{
    /// <summary>
    /// Updates an existing <see cref="WorkflowTask"/> through the Xurrent GraphQL API.<br/>
    /// This cmdlet constructs a <see cref="WorkflowTaskUpdateInput"/> from the provided parameters, executes the operation, and returns a <see cref="WorkflowTaskUpdatePayload"/> describing the result.<br/>
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "XurrentWorkflowTask")]
    [OutputType(typeof(WorkflowTaskUpdatePayload))]
    public class SetXurrentWorkflowTask : XurrentCmdletBase
    {
        /// <summary>
        ...
        public bool? CopyNoteToRequest { get; set; }
        ...
        public bool? CopyNoteToWorkflow { get; set; }
        ...
        /// <summary>
        /// Specifies the <see cref="WorkflowTaskQuery"/> that defines which fields of the <see cref="WorkflowTaskUpdatePayload"/> are returned by the mutation.<br/>
        ...
        /// <summary>
        /// Executes the mutation by constructing a <see cref="WorkflowTaskUpdateInput"/> from the bound parameters, submitting it with the provided or default client, and writing the resulting <see cref="WorkflowTaskUpdatePayload"/> to the pipeline.<br/>
        /// Throws a terminating error if the request fails.<br/>
        /// </summary>
        protected override void OnProcessRecord()
        {
            WorkflowTaskUpdateInput input = new();
            ...
            if (MyInvocation.BoundParameters.ContainsKey(nameof(CopyNoteToRequest)))
                input.CopyNoteToRequest = CopyNoteToRequest;

            if (MyInvocation.BoundParameters.ContainsKey(nameof(CopyNoteToWorkflow)))
                input.CopyNoteToWorkflow = CopyNoteToWorkflow;
            ...
            try
            {
                ...
                WorkflowTaskUpdatePayload result = client.Client.MutationAsync(input, ResponseQuery).GetAwaiter().GetResult();
                WriteObject(result, false);
            }
            catch (XurrentException ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentWorkflowTask), ErrorCategory.NotSpecified, this));
            }
            catch (Exception ex)
            {
                ThrowTerminatingError(new ErrorRecord(ex, nameof(SetXurrentWorkflowTask), ErrorCategory.NotSpecified, this));
